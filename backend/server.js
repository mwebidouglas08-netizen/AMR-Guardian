...
if (buildExists) {
  ...
  console.log('React build not found at:', buildPath, '— API-only mode');
}

// ── AMR DATA ──────────────────────────────────────────────────────────────────
const AMR_DATA = {
  stats: {
    annualDeaths: 1270000, linkedDeaths: 4950000, projected2050: 10000000,
    diagnosticAccessGap: 47, countriesMonitored: 104,
    resistantInfectionRate: '1 in 6', utiResistanceRate: '1 in 3'
  },
  alerts: [
    ...
    { id: 2, date: '2026-05-03', location: 'Mumbai, India', pathogen: 'E. coli', resistanceType: 'ESBL-producing', risk: 'high', cases: 2341, mortality: '4%', description: 'Extended-spectrum beta-lactamase producing E. coli rates exceeding 74% across Mumbai tertiary hospitals.', genomicConfidence: '91.8%' },
    ...
    { id: 4, date: '2026-05-01', location: 'Lagos, Nigeria', pathogen: 'Salmonella Typhi', resistanceType: 'Fluoroquinolone-resistant MDR', risk: 'high', cases: 612, mortality: '3.2%', description: 'Multi-drug resistant typhoid fever. Fluoroquinolone resistance at 88%. Children under 5 primarily affected.', genomicConfidence: '94.5%' },
    { id: 5, date: '2026-04-29', location: 'Jakarta, Indonesia', pathogen: 'S. aureus (MRSA)', resistanceType: 'Methicillin-resistant', risk: 'medium', cases: 331, mortality: '8%', description: 'Community-acquired MRSA rising. Livestock antibiotic overuse suspected as resistance driver.', genomicConfidence: '88.2%' },
    ...
    { id: 7, date: '2026-04-26', location: 'Tashkent, Uzbekistan', pathogen: 'M. tuberculosis', resistanceType: 'Pre-XDR TB', risk: 'medium', cases: 56, mortality: '15%', description: 'Pre-XDR tuberculosis cluster. AMR Guardian flagged anomalous treatment failure rates from 12 clinics.', genomicConfidence: '92.3%' }
  ],
  regionalResistance: [
...
const STEWARDSHIP_DATA = {
  ...
  'Community-acquired pneumonia': { firstLine: { drug: 'Amoxicillin 1g PO three times daily', dose: 'x 5 days (mild) or IV if moderate-severe', safePregnancy: true }, alternatives: ['Amoxicillin-clavulanate 1g PO BD (moderate)', 'Doxycycline 100mg PO BD x 5d (atypical cover)', 'Co-amoxiclav + Clarithromycin (moderate-severe)'], pregnancyFirst: 'Amoxicillin-clavulanate 625mg PO TDS x 7 days', avoid: ['Macrolide monotherapy (pneumococcal resistance >35%)', 'Levofloxacin as first-line'], duration: '5-7 days (mild), 7-10 days (moderate)', warning: 'Macrolide resistance in S. pneumoniae exceeds 35% in some regions. Do not use azithromycin monotherapy.', notes: 'CRB-65 score guides severity. Add atypical coverage for hospitalized patients.' },
...
function getFallbackResponse(msg) {
  ...
  if (m.includes('death') || m.includes('kill') || m.includes('many')) return 'AMR directly causes approximately 1.27 million deaths per year globally - more than HIV/AIDS (~680K) or malaria (~600K). An additional 4.95 million deaths are associated with AMR annually. Projections to 2050: 10 million deaths/year.';
  if (m.includes('africa') || m.includes('lmic')) return 'AMR is far worse in Sub-Saharan Africa due to: weak surveillance (only 18% lab data coverage), over-the-counter antibiotic sales without prescription in 70% of countries, agricultural antibiotic overuse, water and sanitation gaps, and healthcare workforce shortage (1 doctor per 10,000 people).';
  if (m.includes('wastewater') || m.includes('surveillance')) return 'Wastewater-based AMR surveillance samples sewage to detect resistance genes (blaNDM, blaKPC, mcr-1) using qPCR or metagenomic sequencing. One sample represents thousands of people simultaneously. Key advantage: detects emerging resistance 1-4 weeks before clinical cases appear.';
  ...
  if (m.includes('pipeline') || m.includes('new antibiotic')) return 'The antibiotic pipeline is critically thin. WHO lists ~90 agents in development but only 12 have truly novel mechanisms. No new antibiotic class has been discovered since 1987.';
...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    ...
    res.json({ response: response || getFallbackResponse(message) });
  } catch (error) {
...
app.post('/api/reports/generate', async (req, res) => {
  ...
  const prompts = {
    weekly: 'Generate a structured weekly AMR surveillance digest. Include: global death toll estimate, top 5 resistance alerts with location/pathogen/risk level, key regional trends with resistance percentages, emerging resistance genes (NDM-1, KPC, OXA-48, mcr-1), and clinical action items. Use ## for headers.',
    antibiogram: 'Generate a regional antibiogram report for Sub-Saharan Africa Q2 2026. Include: executive summary, resistance rates for E. coli/K. pneumoniae/S. aureus against common antibiotics, recommended empiric therapy for UTI/pneumonia/sepsis, antibiotics to avoid empirically. Use ## for headers.',
    outbreak: 'Generate an outbreak investigation report for carbapenem-resistant Klebsiella pneumoniae (NDM-1) in Nairobi, Kenya. Cases: 147, Mortality: 23%, Detected: May 4 2026. Include: outbreak timeline, genomic findings, containment measures, treatment options. Use ## for headers.'
  };
  if (!prompts[reportType]) return res.status(400).json({ error: 'Invalid report type' });
  try {
    const response = await callGemini('You are AMR Guardian, an AI-powered AMR intelligence system. Generate professional, evidence-based public health reports. Use ## for section headers.', prompts[reportType]);
    res.json({ report: response || getStaticReport(reportType), generated: new Date().toISOString() });
  } catch (error) {
...
app.post('/api/admin/login', (req, res) => {
  ...
  console.log('Admin login attempt for user:', inputUser);
  if (inputUser === validUser && inputPass === validPass) {
    res.json({ success: true, token: Buffer.from(inputUser + ':' + Date.now()).toString('base64'), message: 'Login successful' });
  } else {
    console.log('Login failed. Expected:', validUser);
...
app.get('/admin', (req, res) => {
  res.send(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>AMR Guardian Admin</title><link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500&family=DM+Mono:wght@400;500&family=Syne:wght@700;800&display=swap" rel="stylesheet"><style>*{margin:0;padding:0;box-sizing:border-box}body{background:#0a0c10;color:#e8eaf0;font-family:'DM Sans',sans-serif;font-size:14px;min-height:100vh;display:flex;align-items:center;justify-content:center}.login-box{background:#1c212d;border:1px solid rgba(255,255,255,0.08);border-radius:14px;padding:36px;width:380px}.logo{display:flex;align-items:center;gap:10px;margin-bottom:28px}.logo-icon{width:36px;height:36px;background:linear-gradient(135deg,#00d4a0,#4a9eff);border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:18px}.logo-text{font-family:'Syne',sans-serif;font-weight:800;font-size:16px}.logo-sub{font-size:10px;color:#8891a8;letter-spacing:1px;text-transform:uppercase}h2{font-family:'Syne',sans-serif;font-size:20px;font-weight:700;margin-bottom:6px}.sub{font-size:12px;color:#8891a8;margin-bottom:24px}.form-group{margin-bottom:16px}label{display:block;font-size:11px;color:#8891a8;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:6px;font-family:'DM Mono',monospace}input{width:100%;background:#111318;border:1px solid rgba(255,255,255,0.12);border-radius:8px;padding:10px 14px;color:#e8eaf0;font-family:'DM Sans',sans-serif;font-size:13px;outline:none;transition:border-color 0.2s}input:focus{border-color:#00d4a0}.btn{width:100%;background:#00d4a0;color:#000;border:none;border-radius:8px;padding:11px;font-size:13px;font-weight:600;cursor:pointer;margin-top:8px;transition:opacity 0.2s}.btn:hover{opacity:0.85}.btn:disabled{opacity:0.5;cursor:not-allowed}.err-box{background:rgba(255,71,87,0.12);border:1px solid rgba(255,71,87,0.25);color:#ff6b7a;border-radius:8px;padding:10px 14px;font-size:12px;margin-bottom:14px;display:none}.dw{display:none;width:100%;min-height:100vh;flex-direction:column}.dh{display:flex;align-items:center;justify-content:space-between;padding:18px 28px;background:#111318;border-bottom:1px solid rgba(255,255,255,0.07)}.db{padding:28px;max-width:1100px;margin:0 auto;width:100%}.grid{display:grid;gap:16px}.g4{grid-template-columns:repeat(4,1fr)}.g2{grid-template-columns:1fr 1fr}.card{background:#1c212d;border:1px solid rgba(255,255,255,0.07);border-radius:12px;padding:20px}.cl{font-size:11px;text-transform:uppercase;letter-spacing:1px;color:#8891a8;font-family:'DM Mono',monospace;margin-bottom:8px}.cv{font-family:'Syne',sans-serif;font-size:26px;font-weight:700;line-height:1}.st{font-family:'Syne',sans-serif;font-size:15px;font-weight:600;margin-bottom:14px}table{width:100%;border-collapse:collapse;font-size:13px}th{text-align:left;padding:10px 14px;font-size:11px;text-transform:uppercase;letter-spacing:0.8px;color:#8891a8;font-family:'DM Mono',monospace;font-weight:400;border-bottom:1px solid rgba(255,255,255,0.07)}td{padding:12px 14px;border-bottom:1px solid rgba(255,255,255,0.04)}tr:last-child td{border-bottom:none}.badge{display:inline-flex;align-items:center;padding:3px 8px;border-radius:4px;font-size:11px;font-family:'DM Mono',monospace}.bc{background:rgba(255,71,87,0.15);color:#ff6b7a;border:1px solid rgba(255,71,87,0.2)}.bh{background:rgba(255,140,66,0.15);color:#ffaa77;border:1px solid rgba(255,140,66,0.2)}.bm{background:rgba(255,209,102,0.15);color:#ffd166;border:1px solid rgba(255,209,102,0.2)}.ok{color:#00d4a0}.wn{color:#ffd166}.mn{font-family:'DM Mono',monospace;font-size:12px;color:#8891a8}.lb{background:transparent;color:#8891a8;border:1px solid rgba(255,255,255,0.12);border-radius:8px;padding:7px 16px;font-size:12px;cursor:pointer}.lb:hover{color:#e8eaf0}@media(max-width:800px){.g4{grid-template-columns:1fr 1fr}.g2{grid-template-columns:1fr}.db{padding:16px}}</style></head><body><div id="lp"><div class="login-box"><div class="logo"><div class="logo-icon">🧬</div><div><div class="logo-text">AMR Guardian</div><div class="logo-sub">Admin Panel</div></div></div><h2>Administrator Login</h2><p class="sub">Restricted access — authorised personnel only</p><div class="err-box" id="er"></div><div class="form-group"><label>Username</label><input type="text" id="un" placeholder="Enter username" autocomplete="username"/></div><div class="form-group"><label>Password</label><input type="password" id="pw" placeholder="Enter password" autocomplete="current-password" onkeydown="if(event.key==='Enter')login()"/></div><button class="btn" id="lb" onclick="login()">Sign In</button></div></div><div id="dp" class="dw"><div class="dh"><div class="logo"><div class="logo-icon">🧬</div><div><div class="logo-text">AMR Guardian</div><div class="logo-sub">Admin Dashboard</div></div></div><button class="lb" onclick="logout()">Sign Out</button></div><div class="db"><div class="grid g4" style="margin-bottom:20px" id="sc"></div><div class="grid g2" style="margin-bottom:20px"><div class="card"><div class="st">System Health</div><div id="sh"></div></div><div class="card"><div class="st">Regional Resistance</div><div id="rt"></div></div></div><div class="card"><div class="st">All Active AMR Alerts</div><table><thead><tr><th>Date</th><th>Location</th><th>Pathogen</th><th>Resistance Type</th><th>Risk</th><th>Cases</th><th>Mortality</th></tr></thead><tbody id="at"></tbody></table></div></div></div><script>let tk=null;async function login(){const u=document.getElementById('un').value.trim(),p=document.getElementById('pw').value.trim(),b=document.getElementById('lb'),e=document.getElementById('er');if(!u||!p){e.textContent='Please enter both fields.';e.style.display='block';return;}b.disabled=true;b.textContent='Signing in...';e.style.display='none';try{const r=await fetch('/api/admin/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:u,password:p})});const d=await r.json();if(d.success){tk=d.token;document.getElementById('lp').style.display='none';document.getElementById('dp').style.display='flex';loadDash();}else{e.textContent='Invalid username or password.';e.style.display='block';}}catch(ex){e.textContent='Connection error. Please try again.';e.style.display='block';}finally{b.disabled=false;b.textContent='Sign In';}}function logout(){tk=null;document.getElementById('dp').style.display='none';document.getElementById('lp').style.display='flex';document.getElementById('un').value='';document.getElementById('pw').value='';}async function loadDash(){try{const r=await fetch('/api/admin/overview',{headers:{'Authorization':'Bearer '+tk}});const d=await r.json();const s=d.summary;document.getElementById('sc').innerHTML=[{l:'Total Alerts',v:s.totalAlerts,c:'#4a9eff'},{l:'Critical Alerts',v:s.criticalAlerts,c:'#ff4757'},{l:'High Alerts',v:s.highAlerts,c:'#ff8c42'},{l:'Countries Monitored',v:s.countriesMonitored,c:'#00d4a0'}].map(x=>'<div class="card"><div class="cl">'+x.l+'</div><div class="cv" style="color:'+x.c+'">'+x.v+'</div></div>').join('');const h=d.systemHealth;document.getElementById('sh').innerHTML=[['AI Provider',s.apiStatus,s.apiStatus.includes('Connected')?'ok':'wn'],['Uptime',Math.round(h.uptime)+'s','ok'],['Memory',h.memoryMB+' MB',h.memoryMB<200?'ok':'wn'],['Node Version',h.nodeVersion,'mn'],['Environment',h.environment,'mn'],['Last Updated',new Date(s.lastUpdated).toLocaleTimeString(),'mn']].map(([k,v,c])=>'<div style="display:flex;justify-content:space-between;padding:7px 0;border-bottom:1px solid rgba(255,255,255,0.04);font-size:13px"><span style="color:#8891a8">'+k+'</span><span class="'+c+'">'+v+'</span></div>').join('');document.getElementById('rt').innerHTML='<div style="display:flex;flex-direction:column;gap:7px">'+d.regionalResistance.map(r=>{const col=r.rate>70?'#ff4757':r.rate>50?'#ff8c42':r.rate>30?'#ffd166':'#00d4a0';return'<div style="display:flex;align-items:center;gap:8px;font-size:12px"><span style="width:140px;color:#8891a8;flex-shrink:0">'+r.region+'</span><div style="flex:1;height:5px;background:rgba(255,255,255,0.08);border-radius:3px"><div style="width:'+r.rate+'%;height:100%;background:'+col+';border-radius:3px"></div></div><span style="width:35px;text-align:right;color:'+col+';font-family:DM Mono,monospace">'+r.rate+'%</span></div>';}).join('')+'</div>';document.getElementById('at').innerHTML=d.alerts.map(a=>'<tr><td class="mn">'+a.date+'</td><td>'+a.location+'</td><td style="font-style:italic">'+a.pathogen+'</td><td>'+a.resistanceType+'</td><td><span class="badge '+(a.risk==='critical'?'bc':a.risk==='high'?'bh':'bm')+'">'+a.risk.charAt(0).toUpperCase()+a.risk.slice(1)+'</span></td><td class="mn">'+a.cases.toLocaleString()+'</td><td class="mn">'+a.mortality+'</td></tr>').join('');}catch(ex){console.error('Dashboard error:',ex);}}</script></body></html>`);
});
...
app.get('*', (req, res) => {
  ...
  if (buildExists) {
    ...
    res.status(200).json({ message: 'AMR Guardian API is running. Frontend build not found.' });
  }
});
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('AI: ' + (process.env.GEMINI_API_KEY ? 'Gemini connected' : 'Fallback mode'));
  console.log('Frontend build: ' + (buildExists ? 'Found' : 'Not found'));
